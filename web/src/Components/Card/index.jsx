export const Card = ({ timeA, timeB, match }) => (
    <div className="rounded-xl border border-gray-300 text-center p-4 space-y-4">
        <span className='text-xs md:text-md text-gray-700 font-bold'>{match.time}</span>
        <div className="flex space-x-4 justify-center items-center">
            <span className="uppercase">{timeA.slug}</span>
            <img src={`/imgs/flags/${timeA.slug}.png`} />

            <input type="number" max={10} className='bg-red-300/[0.2] w-[55px] h-[55px] text-red-700 text-xl text-center' />
